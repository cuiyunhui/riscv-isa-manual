...
[[ext:svnapot16k]]
[[ext:svnapot32k]]
The SvnapotNk extension family defines the independently discoverable
ext:svnapot16k[] and ext:svnapot32k[] ISA extension strings.
[#norm:Svnapot16k_depends_Svnapot]#The ext:svnapot16k[] extension depends on
Svnapot# and [#norm:Svnapot16k_encoding_valid]#makes the level-0 encoding
with _pte_.__napot_bits__=2 valid for a 16 KiB contiguous region#.
[#norm:Svnapot32k_depends_Svnapot]#The ext:svnapot32k[] extension depends on
Svnapot# and [#norm:Svnapot32k_encoding_valid]#makes the level-0 encoding
with _pte_.__napot_bits__=3 valid for a 32 KiB contiguous region#.
...
`x xxxx xxxx`
|_Reserved_ +
16 KiB contiguous region if `Svnapot16k` is implemented; _Reserved_ otherwise +
32 KiB contiguous region if `Svnapot32k` is implemented; _Reserved_ otherwise +
...
In particular, [#norm:Svnapot16k_unsupported_encoding_fault]#the
_pte_.__napot_bits__=2 encoding is reserved when `Svnapot16k` is not
implemented#, and [#norm:Svnapot32k_unsupported_encoding_fault]#the
_pte_.__napot_bits__=3 encoding is reserved when `Svnapot32k` is not
implemented#.
...
Depending on need, the NAPOT scheme may be extended to other page sizes
and/or to other levels of the page table in the future. The encoding is
designed to accommodate other NAPOT sizes should that need arise. For
example:
...
Each additional size requires an architectural discovery mechanism so
system software can determine which encodings are supported.
...
The 16 KiB and 32 KiB sizes are therefore independently discoverable and
...
translation#.
The `Svnapot16k` and `Svnapot32k` encodings follow the same G-stage rule.
